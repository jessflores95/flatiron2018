...
def full_rand(nx, ny, nz, initial_perturbation_size):
    ...
    rand = np.exp(1j*kz*z)*intial_perturbation_size*nx*ny*nz/(2*np.pi)
    rand[0][:] = 0.0
    rand[1][:] = 0.0
    #rand[5:-5,:,:] = 0.0
    #rand[:,5:-5,:] = 0.0
...
##        Dh_tilde = fft3(D_tilde)
        ...
        Dh00_total[i_ind] = np.sqrt(((np.abs(D_tilde[0,0]))**2).sum()*xh*yh*zh)
        Dh01_total[i_ind] = np.sqrt(((np.abs(D_tilde[0,1]))**2).sum()*xh*yh*zh)
        Dh02_total[i_ind] = np.sqrt(((np.abs(D_tilde[0,2]))**2).sum()*xh*yh*zh)
        Dh10_total[i_ind] = np.sqrt(((np.abs(D_tilde[1,0]))**2).sum()*xh*yh*zh)
        Dh11_total[i_ind] = np.sqrt(((np.abs(D_tilde[1,1]))**2).sum()*xh*yh*zh)
        Dh12_total[i_ind] = np.sqrt(((np.abs(D_tilde[1,2]))**2).sum()*xh*yh*zh)
        Dh20_total[i_ind] = np.sqrt(((np.abs(D_tilde[2,0]))**2).sum()*xh*yh*zh)
        Dh21_total[i_ind] = np.sqrt(((np.abs(D_tilde[2,1]))**2).sum()*xh*yh*zh)
        Dh22_total[i_ind] = np.sqrt(((np.abs(D_tilde[2,2]))**2).sum()*xh*yh*zh)


##        Dh_off = -(gamma+omega_k)*D_tilde
##        Dh_diag = -omega_k*D_tilde
##
...
kesi_loop = np.linspace(0.0,1.0,10)

for k_ind,k in enumerate(kesi_loop):
    kesi = kesi_loop[k_ind]
    Dh00_total, Dh01_total,Dh02_total,Dh10_total,Dh11_total,Dh12_total,Dh20_total,Dh21_total,Dh22_total = simulation(Dh)
    
    file_Dh00_total = '/mnt/home/jflores/Doi-OnsagerModel/bifurcations_3d/kesi_vary/kesi_'+str(k_ind)+'/Dh00_total.txt'
    file_Dh01_total = '/mnt/home/jflores/Doi-OnsagerModel/bifurcations_3d/kesi_vary/kesi_'+str(k_ind)+'/Dh01_total.txt'
    file_Dh02_total = '/mnt/home/jflores/Doi-OnsagerModel/bifurcations_3d/kesi_vary/kesi_'+str(k_ind)+'/Dh02_total.txt'
    file_Dh10_total = '/mnt/home/jflores/Doi-OnsagerModel/bifurcations_3d/kesi_vary/kesi_'+str(k_ind)+'/Dh10_total.txt'
    file_Dh11_total = '/mnt/home/jflores/Doi-OnsagerModel/bifurcations_3d/kesi_vary/kesi_'+str(k_ind)+'/Dh11_total.txt'
    file_Dh12_total = '/mnt/home/jflores/Doi-OnsagerModel/bifurcations_3d/kesi_vary/kesi_'+str(k_ind)+'/Dh12_total.txt'
    file_Dh20_total = '/mnt/home/jflores/Doi-OnsagerModel/bifurcations_3d/kesi_vary/kesi_'+str(k_ind)+'/Dh20_total.txt'
    file_Dh21_total = '/mnt/home/jflores/Doi-OnsagerModel/bifurcations_3d/kesi_vary/kesi_'+str(k_ind)+'/Dh21_total.txt'
    file_Dh22_total = '/mnt/home/jflores/Doi-OnsagerModel/bifurcations_3d/kesi_vary/kesi_'+str(k_ind)+'/Dh22_total.txt'
