...
def L2_diff(Dh_t,Dh_exp,xh,yh,zh):
    return np.sqrt(((np.abs(Dh_t-Dh_exp))**2).sum()*xh*yh*zh)
...
err_Dh = np.zeros((n_timesteps),dtype=float)
...
for i_ind,i in enumerate(ns):
    ...
    for j in range(3):
        for l in range(3):
            if j == l:
                Dh_exp[j,l] = -omega_k*Dh_tilde[j,l]
            else:
                Dh_exp[j,l] = -(gamma+omega_k)*Dh_tilde[j,l]
    ...
    # compute derivative: Dh(t+dt)-Dh(t)/dt
    Dh_t = (Dh - Dh_tilde)/dt
    #compute L2 norm of difference between Dh_t and Dh_exp=omega_k*Dh_tilde
    #This difference should be very small close to 0 
    err_Dh[i_ind] = L2_diff(Dh_t,Dh_exp,xh,yh,zh)
...
file_err_Dh = '/mnt/home/jflores/Doi-OnsagerModel/plane_wave_pert_z/D_pert/single_mode/err_Dh.txt'

np.savetxt(file_err_Dh,err_Dh)
